...
namespace SciFiShopWebAPI.Controllers
{
  ...
  public class AccountController : ControllerBase
  {
    ...
    // localhost:5135/api/account/register
    [HttpPost("register")]
    public async Task<IActionResult> Register(LoginRequestDto loginRequestDto)
    {
      if (await _unitOfWork.UserRepository.UserAlreadyExists(loginRequestDto.Username))
      {
        return BadRequest("This username is already taken, please try something else");
      }
